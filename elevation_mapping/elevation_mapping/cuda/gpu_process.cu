...
__global__ void G_Printf_map() {
    ...
    printf("P:central_coordinate:%f,%f\n", central_coordinate[0],
           central_coordinate[1]);
}
...
// inline static
__device__ Eigen::Matrix<PrimType_, 3, 3>
GetSkewMatrixFromVector(const Eigen::Matrix<PrimType_, 3, 1> &vec) {
...
__global__ void G_pointsprocess(int *map_index, float *point_x, float *point_y, float *point_z, float *result_var, float *point_x_ts, float *point_y_ts, float *point_z_ts, Eigen::Matrix4f transform, int point_num, Eigen::RowVector3f C_sensorJacobian, Eigen::Matrix3f C_rotationVariance, Eigen::Matrix3f C_C_SB_transpose, Eigen::RowVector3f C_P_mul_C_BM_transpose, Eigen::Matrix3f C_B_r_BS_skew) {
    int i = blockDim.x * blockIdx.x + threadIdx.x;
    if (i < point_num) {
        float point_height = transform(2, 0) * point_x[i] +
                             transform(2, 1) * point_y[i] +
                             transform(2, 2) * point_z[i] + transform(2, 3);
        ...
        // // sim old TWILIGHT DEBUG
        // if ((point_x[i] > -0.5 && point_x[i] < 0.5) && (point_y[i] > -0.6 && point_y[i] < 0.6)) // pandar x left y back
        // {
        //     flag = 1;
        // }

        if ((point_x[i] > -1 && point_x[i] < 0) &&
            (point_y[i] > -0.4 && point_y[i] < 0.4) &&
            (point_z[i] > 0 && point_z[i] < 0.3)) {
            flag = 1;
        }
        ...
        if ((point_height > C_relativeLowerThreshold &&
            ...
            flag == 0) {
            point_x_ts[i] = transform(0, 0) * point_x[i] +
                            transform(0, 1) * point_y[i] +
                            transform(0, 2) * point_z[i] + transform(0, 3);
            point_y_ts[i] = transform(1, 0) * point_x[i] +
                            transform(1, 1) * point_y[i] +
                            transform(1, 2) * point_z[i] + transform(1, 3);
            point_z_ts[i] = point_height;

            Eigen::Vector3f pointVector(point_x[i], point_y[i],
                                        point_z[i]); // S_r_SP
            ...
            float varianceLateral =
                pow(C_beam_c + C_beam_a * measurementDistance, 2);
            ...
            const Eigen::Matrix3f C_SB_transpose_times_S_r_SP_skew =
                SkewMatrixFromVector(
                    Eigen::Vector3f(C_C_SB_transpose * pointVector));
            Eigen::RowVector3f rotationJacobian =
                C_P_mul_C_BM_transpose *
                (C_SB_transpose_times_S_r_SP_skew + C_B_r_BS_skew);

            // Measurement variance for map (error propagation law).
            Eigen::Vector3f rotationJacobian_T =
                cuda_Transpose(rotationJacobian);
            heightVariance = cuda_computer(rotationJacobian, C_rotationVariance,
                                           rotationJacobian_T);

            Eigen::Vector3f C_sensorJacobian_T =
                cuda_Transpose(C_sensorJacobian);
            heightVariance += cuda_computer(C_sensorJacobian, sensorVariance,
                                            C_sensorJacobian_T);
            ...
            if (grid_index != -1) {
                ...
                if (point_height == map_lowest[grid_index]) {
                    map_lowest[grid_index] =
                        map_lowest[grid_index] + 3 * heightVariance;
                }
            }
        } else {
            map_index[i] = -1;
...
__device__ float d_min_elevation(int cell_index_x, int cell_index_y,
                                 int obstacle_index_x, float robot_index_x) {
    ...
    if (i < Length * Length) {
        if (map_traver[i] < obstacle_threshold && map_elevation[i] != -10) {
            ...
            if (increment_x == 0 && increment_y == 0)
                ...
                while (current_indice[1] >= 0 && current_indice[1] < Length) {
                    if (P_isVaild(current_indice[0], current_indice[1])) {
                        obstacle_max_ele = d_min_elevation(
                            current_indice[0], current_indice[1],
                            obstacle_indice[0], robot_index);
                        // std::cout << "x:"<<current_indice[0] << " y:" <<
                        // current_indice[1]<< std::endl;
            ...
            if (fabs(increment[0]) > fabs(increment[1]))
                ...
                threshold =
                    sqrt(0.5 * 0.5 + pow(0.5 / increment[1] * increment[0], 2));
            ...
            // obstacle_restrict_ele); d_map_clean[i] = obstacle_restrict_ele;
            if (obstacle_ele - 3 * sqrt(map_variance[i]) >
                obstacle_restrict_ele)
                map_elevation[i] = -10;
        }
        // else
        //     d_map_clean[i] = 1;
...
bool getIndexShiftFromPositionShift(int *indexShift, float *positionShift,
                                    float resolution) {
    for (int i = 0; i < 2; i++) {
        indexShift[i] = static_cast<int>(positionShift[i] / resolution +
                                         0.5 * (positionShift[i] > 0 ? 1 : -1));
    }
    // Debug
    // std::cout << "indexShift1:" << indexShift[0]<< "   indexShift2:" <<
    // indexShift[1]<< std::endl;
    return true;
}

bool getPositionShiftFromIndexShift(float *positionShift, int *indexShift,
                                    float resolution) {
    for (int i = 0; i < 2; i++) {
...
void Init_GPU_elevationmap(int length, float resolution,
                           float h_mahalanobisDistanceThreshold_,
                           float h_obstacle_threshold) {
    ...
    cudaMemcpyToSymbol(map_intensity, &h_map_intensity, sizeof(float *),
                       size_t(0), cudaMemcpyHostToDevice);
    cudaMemcpyToSymbol(map_elevation, &h_map_elevation, sizeof(float *),
                       size_t(0), cudaMemcpyHostToDevice);
    cudaMemcpyToSymbol(map_variance, &h_map_variance, sizeof(float *),
                       size_t(0), cudaMemcpyHostToDevice);
    cudaMemcpyToSymbol(map_lowest, &h_map_lowest, sizeof(float *), size_t(0),
                       cudaMemcpyHostToDevice);
    cudaMemcpyToSymbol(map_traver, &h_map_traver, sizeof(float *), size_t(0),
                       cudaMemcpyHostToDevice);
    cudaMemcpyToSymbol(map_colorR, &h_map_colorR, sizeof(int *), size_t(0),
                       cudaMemcpyHostToDevice);
    cudaMemcpyToSymbol(map_colorG, &h_map_colorG, sizeof(int *), size_t(0),
                       cudaMemcpyHostToDevice);
    cudaMemcpyToSymbol(map_colorB, &h_map_colorB, sizeof(int *), size_t(0),
                       cudaMemcpyHostToDevice);
    ...
    int threadsPerBlock = 256;
    int blocksPerGrid =
        (length * length + threadsPerBlock - 1) / threadsPerBlock;
...
int Process_points(int *map_index, float *point_x, float *point_y, float *point_z, float *point_var, float *point_x_ts, float *point_y_ts, float *point_z_ts, Eigen::Matrix4f transform, int point_num, double relativeLowerThreshold, double relativeUpperThreshold, float min_r, float beam_a, float beam_c, Eigen::RowVector3f sensorJacobian, Eigen::Matrix3f rotationVariance, Eigen::Matrix3f C_SB_transpose, Eigen::RowVector3f P_mul_C_BM_transpose, Eigen::Matrix3f B_r_BS_skew) {
    ...
    cudaMemcpy(dev_x, point_x, point_num * sizeof(float),
               cudaMemcpyHostToDevice);
    cudaMemcpy(dev_y, point_y, point_num * sizeof(float),
               cudaMemcpyHostToDevice);
    cudaMemcpy(dev_z, point_z, point_num * sizeof(float),
               cudaMemcpyHostToDevice);

    cudaMemcpyToSymbol(C_relativeLowerThreshold, &relativeLowerThreshold,
                       sizeof(double));
    cudaMemcpyToSymbol(C_relativeUpperThreshold, &relativeUpperThreshold,
                       sizeof(double));
    ...
    int threadsPerBlock = 256;
    int point_blocksPerGrid =
        (point_num + threadsPerBlock - 1) / threadsPerBlock;
    G_pointsprocess<<<point_blocksPerGrid, threadsPerBlock>>>(
        dev_mapindex, dev_x, dev_y, dev_z, dev_result, dev_x_ts, dev_y_ts,
        dev_z_ts, transform, point_num, sensorJacobian, rotationVariance,
        C_SB_transpose, P_mul_C_BM_transpose, B_r_BS_skew);

    cudaMemcpy(point_var, dev_result, point_num * sizeof(float),
...
void Fuse(int length, int point_num, int *point_index, int *point_colorR, int *point_colorG, int *point_colorB, float *point_intensity, float *point_height, float *point_var) {
    ...
    cudaMemcpy(dev_pointcolorR, point_colorR, point_num * sizeof(int),
               cudaMemcpyHostToDevice);
    cudaMemcpy(dev_pointcolorG, point_colorG, point_num * sizeof(int),
               cudaMemcpyHostToDevice);
    cudaMemcpy(dev_pointcolorB, point_colorB, point_num * sizeof(int),
               cudaMemcpyHostToDevice);
    cudaMemcpy(dev_pointindex, point_index, point_num * sizeof(int),
               cudaMemcpyHostToDevice);
    cudaMemcpy(dev_pointheight, point_height, point_num * sizeof(float),
               cudaMemcpyHostToDevice);
    cudaMemcpy(dev_pointvar, point_var, point_num * sizeof(float),
               cudaMemcpyHostToDevice);
    ...
    int threadsPerBlock = 256;
    int blocksPerGrid = (cell_num + threadsPerBlock - 1) / threadsPerBlock;
    G_fuse<<<blocksPerGrid, threadsPerBlock>>>(
        dev_pointindex, dev_pointcolorR, dev_pointcolorG, dev_pointcolorB,
        dev_pointintensity, dev_pointheight, dev_pointvar, point_num);
...
void Map_feature(int length, float *elevation, float *var, int *colorR, int *colorG, int *colorB, float *rough, float *slope, float *traver, float *intensity) {
    ...
    int threadsPerBlock = 256;
    int blocksPerGrid = (cell_num + threadsPerBlock - 1) / threadsPerBlock;
    G_Mapfeature<<<blocksPerGrid, threadsPerBlock>>>(
        d_colorR, d_colorG, d_colorB, d_elevation, d_var, d_rough, d_slope,
        d_traver, d_intensity);

    cudaMemcpy(colorR, d_colorR, cell_num * sizeof(int),
               cudaMemcpyDeviceToHost);
    cudaMemcpy(colorG, d_colorG, cell_num * sizeof(int),
               cudaMemcpyDeviceToHost);
    cudaMemcpy(colorB, d_colorB, cell_num * sizeof(int),
               cudaMemcpyDeviceToHost);
    cudaMemcpy(elevation, d_elevation, cell_num * sizeof(float),
               cudaMemcpyDeviceToHost);
    cudaMemcpy(var, d_var, cell_num * sizeof(float), cudaMemcpyDeviceToHost);
    cudaMemcpy(rough, d_rough, cell_num * sizeof(float),
               cudaMemcpyDeviceToHost);
    cudaMemcpy(slope, d_slope, cell_num * sizeof(float),
               cudaMemcpyDeviceToHost);
    cudaMemcpy(traver, d_traver, cell_num * sizeof(float),
               cudaMemcpyDeviceToHost);
    cudaMemcpy(intensity, d_intensity, cell_num * sizeof(float),
               cudaMemcpyDeviceToHost);
